...
class FoolGameEngine:
    ...
    def get_throwable_cards(self, player_id: int) -> List[Card]:
        if not self.table or len(self.table) >= 6:
            return []

        table_ranks = {c[0].rank for c in self.table if c[0]}
        return [card for card in self.hands.get(player_id, [])
                if card and card.rank in table_ranks]
    ...
    def get_state(self, player_id: int) -> Dict:
        ...
        return {
            ...
            'throwable_cards': self.get_throwable_cards(player_id)
        }
    ...
    def _get_allowed_actions(self, player_id: int) -> List[str]:
        ...
        if player_id == self.attacker:
            ...
            for i, card in enumerate(self.hands[player_id]):
                if card and (not self.table or card.rank in table_ranks):
                    actions.append(str(i))
            if self.table and all(card[1] for card in self.table):
                actions.append("pass")

        elif player_id == self.defender and self.table:
            # Находим первую неотбитую карту
            attack_card = next((card[0] for card in self.table if card[1] is None), None)
            if attack_card:
                for i, card in enumerate(self.hands[player_id]):
    ...
    def _process_defense(self, player_id: int, action: str) -> bool:
        ...
        try:
            ...
            if 0 <= card_idx < len(self.hands[player_id]):
                ...
                if (defending_card and attacking_card and
                        self._can_beat(attacking_card, defending_card)):
                    self.table[table_index] = (attacking_card, defending_card)
                    self.hands[player_id].pop(card_idx)
